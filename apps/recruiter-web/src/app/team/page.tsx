...
export default function TeamPage() {
  ...
  // ponytail: no list-recruiter-invitations procedure; links live in state for this session only
  const [links, setLinks] = useState<{ email: string; url: string }[]>([]);

  const onInvite = async () => {
    const { inviteRecruiter } = await invite.mutateAsync({ email });
    const url = `${window.location.origin}/accept-invite?token=${inviteRecruiter.token}`;
    setLinks((l) => [{ email, url }, ...l]);
    setEmail("");
  };
